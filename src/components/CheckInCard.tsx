...
const CheckInCard = () => {
  ...
  const handleCheckIn = () => {
    ...
    const alreadyCheckedIn = activeCheckIns.some(
      (record) => record.employeeName.toLowerCase() === employeeName.trim().toLowerCase()
    );
    ...
    toast({
      title: "Checked In!",
      description: `Welcome, ${newRecord.employeeName}! Your check-in time has been recorded.`,
    });
  ...
  const handleCheckOut = (recordId: string) => {
    ...
    if (record) {
      ...
      toast({
        title: "Checked Out!",
        description: `${record.employeeName}, have a great day!`,
      });
        ...
        <CardContent className="pt-6 space-y-6">
          ...
          {activeCheckIns.length > 0 && (
            <div className="space-y-3 pt-4 border-t border-border">
              <p className="text-sm font-medium text-muted-foreground">Currently Checked In</p>
